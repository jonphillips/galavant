...
- **Ops.** `TripIdea.createFreeform` / `editFreeform`, stop-ID-keyed;
  ...
  a freeform stop can also be an **alternative** — "picnic we packed" as an option alongside a
  restaurant — by carrying the alternatives-ring columns like any `TripIdea`.)*
